...
//-------------------------
// Helper to reset speaker pin for normal PWM output
void resetSpeakerPin() {
  pinMode(SPEAKER_PIN, OUTPUT);
  ledcAttachPin(SPEAKER_PIN, SPEAKER_CH);
  ledcWrite(SPEAKER_CH, 0);  // stop sound
}
...
void TaskPlayMusic(void* pvParameters) {
  ...
  esp_task_wdt_init(params->wdTimer, false);  // disable WDT
  esp_task_wdt_add(NULL);
  resetSpeakerPin();  // ensures PWM ready for next use
  
  player.play_nes(params->song, params->loop, params->vol);  // play once

  esp_task_wdt_delete(NULL);// remove watchdgo
  esp_task_wdt_deinit();
  delete params;
  // --- cleanup ---
  resetSpeakerPin();  // ensures PWM ready for next use
  ...
  // mark task handle NULL before self-delete
  musicTaskHandle = NULL;
  vTaskDelete(NULL);  // safely delete self
}

// Start play music
void startMusicPlayback(const uint8_t* song, bool loop, float vol, int wdTimer) {
  auto* params = new MusicParams{ song, loop, vol, wdTimer };
...
void stopMusicPlayback() {
    if (player.NES_PLAYING) {
        player.NES_PLAYING = false;  // signal playback loop to exit gracefully
        vTaskDelay(50 / portTICK_PERIOD_MS);  // short delay to let it break loop
